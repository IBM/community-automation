...
echo "Checking for $default_sc Storage Class"
scCounter=0
rc=1
until [ $rc -eq 0 ]
do
  echo "Waiting for $default_sc storageclass to come up $(date)"
  sleep 10
  ((scCounter ++))
  if [ $scCounter -eq 60 ]; then
   echo "$default_sc storageclass never became available."
   exit 1
  fi
  oc get sc --no-headers | cut -f1 -d' ' | grep $default_sc >/dev/null
  rc=$?
done
